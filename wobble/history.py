...
class History(object):
    ...
    """   
    def __init__(self, model, data, r, niter):
        self.r = r
        self.niter = niter
        self.data = data
        self.nll_history = np.empty(niter)
        self.synth_history = np.empty(np.append(niter, np.shape(data.ys[r])))
        self.rvs_history = [np.empty((niter, data.N)) for c in model.components]
        self.ivars_history = [np.empty((niter, data.N)) for c in model.components]
    ...
    def save_plots(self, basename, epochs=[0,50]):
        ...
        ax = plt.gca()
        ax.set_yscale('log')
        plt.savefig(basename+'_order{0}_nll.png'.format(self.r))   
        plt.clf()
        
        rvs_ani_star = self.plot_rvs(0, compare_to_pipeline=True)
        rvs_ani_star.save(basename+'_order{0}_rvs_star.mp4'.format(self.r), fps=30, extra_args=['-vcodec', 'libx264'])
        
        template_ani_star = self.plot_template(0, nframes=50)
        template_ani_star.save(basename+'_order{0}_template_star.mp4'.format(self.r), fps=30, extra_args=['-vcodec', 'libx264'])
        template_ani_t = self.plot_template(1, nframes=50)
        template_ani_t.save(basename+'_order{0}_template_t.mp4'.format(self.r), fps=30, extra_args=['-vcodec', 'libx264'])

        for e in epochs:
            synth_ani = self.plot_synth(e)
            synth_ani.save(basename+'_order{0}_synth_epoch{1}.mp4'.format(self.r, e), fps=30, extra_args=['-vcodec', 'libx264'])