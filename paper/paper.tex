...
\label{tbl:51peg}
\startdata
$K$ & m s$^{-1}$ & $56.30^{+2.42}_{-2.05}$ & $54.02^{+1.84}_{-1.89}$ \\
$P$ & days & $4.2289^{+0.0004}_{-0.0004}$ & $4.2293^{+0.0003}_{-0.0003}$ \\
$t_0$ & JD & $2456543.85^{+0.44}_{-0.47}$ & $2456546.41^{+0.27}_{-3.78}$ \\
$e$ & $-$  & $0.03^{+0.02}_{-0.02}$ & $0.04^{+0.02}_{-0.02}$ \\
$\omega$ & rad & $0.91^{+4.99}_{-0.64}$ & $4.51^{+0.32}_{-0.35}$ \\
$s$ & m s$^{-1}$ & $0.01^{+0.10}_{-0.01}$ & $0.72^{+0.07}_{-0.06}$ \\
\enddata
\end{deluxetable*}
...
\caption{Three example echelle orders at a randomly chosen epoch from the \HARPS observations of 51 Pegasi. \replaced{Data are shown as dark circles, while the \wobble predictions for the stellar and telluric spectral contributions at this epoch are red and blue lines, respectively.}{Data and best-fit models are plotted in black, while the predicted stellar and telluric spectral contributions from the model fit are plotted in red and blue with arbitrary flux offsets for clarity.} Even in the presence of a strong stellar absorption feature such as the H$\alpha$ line (seen at 6561.5 \ang due to the star's Doppler shift at the plotted epoch), small telluric features are clearly recovered. Residuals after subtracting both star and telluric models are shown below each spectrum.}
...
It is worthy of note that the stellar spectral model shown in Figure \ref{51peg_multispectrum} is an extremely high-\SNR spectrum of 51 Peg. 
...
\replaced{These hypotheses}{This hypothesis} could be tested in the future by seeking correlations between basis weights and externally-measured weather diagnostics. 
...
\replaced{Realistically speaking, if \LSF variations are present they are probably not sufficiently well-understood to take this approach; instead, we might want to fit these variations as a data-driven model component.}{In fact, it is worth noting at this point that the \wobble method does not deal correctly with the instrumental \LSF even for a perfectly stable spectrograph; the equations presented in this work disentangle \LSF-broadened telluric and stellar templates from each other, whereas in reality those templates should be combined \textit{before} the \LSF is applied. For the purposes of this work, where the \LSF is assumed to be time-invariant, the method demonstrably works despite this flaw. However, for other applications the \LSF variations might need to be included as a data-driven model component within a second interpolation operator that acts on the sum of the stellar and telluric model predictions.}
