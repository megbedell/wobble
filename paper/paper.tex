...
\todo{is this true? See Erdmann et al. 1807.08734}
...
(\todo{DFM says: what does ``largely'' mean here?}), this means that no physical knowledge about the object being observed is needed to extract its \RVs.
...
If the number of spectra is small or if all spectra are taken over a very small range of time (e.g. a single night), it will be nearly impossible to disentangle the components of the model, as this process relies on the apparent Doppler shifting of the star(s) and Earth's atmosphere relative to each other. \todo{We should think about whether this is the optimal way to phrase this. A short period SB1 would have no problem disentangling telluric features from spectral features, for example!}
...
\todo{DFM says: feel free to cut this:} $P$ could be represented as a rectangular
...
After subtracting the \RVs as predicted by the equations of motion at each epoch, the residuals have low dispersion, as expected for a quiet star with no known planets. \todo{what about a quiet star with no known planets with K > X m/s?} 
...
Thus the \RV scatter derived in this work is only an approximate upper limit on the true precision achievable by \wobble. \todo{(TO DO: recalculate \BERV ourselves)} \todo{How does HARPS calculate the BERVs? If there is something like cirrus, where the apparent brightness of the star varies over the observation, it is plausible that the flux-weighted observation midpoint and the actual time midpoint are different enough to cause a m/s difference in the observations, too. Does HARPS have a way of accounting for this? (HIRES does estimate this, and they have shorter exposures on average).}
...
\todo{DFM says: when I look at figure 5, I feel like the two spectra really don't
look similar. I think this could use some more discussion about why we find
this figure satisfying.}
...
While this is a useful approximation in the case of the \RM, in general the confusion of these two spectral changes is a major cause of correlated noise in \RV time series \citep[e.g.][]{}. \todo{citep}
...
In principle, if the changes to the \LSF were well-understood they could be hard-coded into the P operators. \todo{This is the first time you've referred to ``p operators'' using that phrasing. Maybe include a callback here to equation 2 so the reader is reminded what you're talking about?}
...
optimization speed would be lost \todo{DFM says: this is not obvious to me.
We're not currently dependent on anything being convex. I don't think we
should say this without trying it...}.
...
Widely used methods of extracting 1D spectra almost certainly sacrifice some degree of spectral information \citep{Bolton2010}. \todo{Maybe this was a recently fixed change, but the pdf you sent didn't show this reference properly}
