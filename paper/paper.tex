...
We may then initialize the stellar spectrum $\mu_{\star}$ by Doppler-shifting the data and calculating the median flux across \BERV-corrected spectra in bins at each model wavelength $\xi_{\star}$. 
...
The $K$ highest eigenweights and their corresponding eigenvectors are taken as the starting guess for the basis weights and vectors. 
...
For the purposes of this work, we chose to test the performance of the \wobble method on archival spectra from the High Accuracy Radial Velocity Planet Searcher (\HARPS) spectrograph \citep{Mayor2003}. 
...
These data come as extracted order-by-order 1D spectra in blocks of 72 orders by 4096 pixels per order.
