...
One of the primary contributors to the noise budget in \RV measurements is the incomplete treatment of telluric features in the Earth's atmosphere \citep{Halverson2016}. 
...
Technically a small bias is introduced by taking the logarithm of the flux, but this bias is very small at the relevant signal-to-noise ratios; furthermore, the small bias \changed{in flux is unlikely to map onto the radial velocities obtained.}%acts in the flux direction, not the radial-velocity direction.
...
\changed{The uncertainties on this spectrum can be estimated from the negative inverse Hessian, similarly to the \RVs. Doing this calculation for a subset of the template in the continuum around 6000 \ang indicates an approximate \SNR of \todo{xxx}, which is consistent with the expected \SNR from co-adding each individual spectrum. }
...
\acknowledgements{\changed{We gratefully acknowledge the referee, Ian Czekala, for a thoughtful and constructive review.}
