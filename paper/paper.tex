...
Data analysis pipelines for absorption cell instruments have long used a stellar template that is customized to the star in question, although traditionally this template is \added{derived from} a single observation taken for this purpose \citep[e.g.][]{Butler1996}. 
...
Our approach \added{to the \EPRV problem} is similar to that taken by \citet{Gao2016}, but differs in that we take the stellar spectrum as a latent variable to be optimized rather than iteratively determining it by stacking model residuals. 
\added{Similar approaches of learning template spectra for stars and tellurics from the data directly have been used in the literature, although they have not previously been demonstrated to achieve precise stellar \RVs \citep[e.g.]{Hadrava2004, Hadrava2006}.}
...
\added{Similarly, we assume that the line spread function of the instrument remains perfectly constant from one exposure to the next. This assumption is needed because it allows us to extract instrumentally-broadened template spectra rather than explicitly modeling and solving for instrumental broadening effects. However, \todo{as we discuss in Section \ref{s:future}}, even within the limits of this assumption extracting already-broadened templates is not strictly correct. We do this for simplicity only and leave the general case to future work.}
...
We also have a corresponding $M \times N$ matrix of wavelength solutions\added{ which we call $\xi$, where each entry $\xi_{m,n}$ is the logarithm of the wavelength for pixel $m$ at epoch $n$}.
\deleted{Again, the entries of this matrix are in logarithmic form ($\ln \lambda_{m,n}$). This choice makes the Doppler shift an additive term, considerably simplifying the math to follow.}

\replaced{We model each data column $y_n$}{For each data column $y_n$, our model prediction $f_n$ can be treated} as the sum of stellar and telluric contributions:
\begin{equation}
...
The template wavelength grid $\xi_{t}$ should have similar properties to $\xi_{\star}$. 
...
\added{The uncertainties on this spectrum can be estimated from the negative inverse Hessian, similarly to the \RVs. Doing this calculation for a subset of the template in the continuum around 6000 \ang indicates an approximate \SNR of \todo{xxx}, which is consistent with the expected \SNR from co-adding each individual spectrum. }
\replaced{This template spectrum is essentially a time-averaged composite spectrum.}{In essence, this template is a time-averaged and telluric-cleaned composite stellar spectrum.} 
