...
We take the data to be the $M \times N$ matrix Y, where each entry $y_{m,n}$ is the observed logarithmic flux $\ln f$ for pixel $m$ of $M$ at epoch $n$ of $N$. orange
...
$m_p$ sin$i$ & $M_J$ & $0.41^{+0.02}_{-0.02}$ & $0.40^{+0.01}_{-0.02}$ \\
\enddata
\end{deluxetable*}
...
The resulting best-fit parameters for the \wobble\ \RVs are generally consistent with expectations based on literature values (Table \ref{tbl:51peg}). 
