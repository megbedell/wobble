...
Next, the templates are held fixed and the time-dependent parameters (stellar \RVs and telluric basis weights) are varied to again maximize the likelihood function. 
...
\todo{(say something about calculating uncertainties here?)}
...
The necessary optimizations over many parameters can be performed with high efficiency by \TF: the below-described analysis of \todo{80} spectra of 51 Peg runs in \todo{30} minutes on a standard Mac desktop.
...
51 Pegasi is a Sun-like star hosting a planet with an orbital period of 4 days and a mass of 0.5 Jupiter masses \citep{Mayor1995}. 
...
\wobble spectral fit in Figure \ref{fig:51peg_spectrum}. It's sweet!

Planet recovered in \RV in Figure \ref{fig:51peg_planet}.

\begin{figure}
\centering
\includegraphics[width=5in]{51peg_multispectrum}
...
This trend has been observed in some data sets, including \acronym{UVES} observations and the \HARPS spectra as analyzed by the standard instrument pipeline, but is not seen in others such as data from \acronym{HIRES} and the same \HARPS spectra as analyzed by the \HARPS-TERRA pipeline \citep{Kurster2003, Bonfils2013, Choi2013, AngladaEscude2012}. 
With or without a secular trend, no planets have been found around \Mdwarf, despite considerable amounts of \RV monitoring. 
...
Spectral fit in Figure \ref{fig:barnards_spectrum}.

Comparison to spectral model prediction in Figure \ref{fig:barnards_model}: even in the blue where individual spectra are hugely noisy, we are seeing real features in the composite spectral template. This is a huge improvement over most existing M dwarf data and may be highly valuable for spectral characterization.

RVs in Figure \ref{fig:barnards_rvs}.

\begin{figure}
\centering
\includegraphics[width=5in]{barnards_multispectrum}
...
\caption{Radial velocity measurements for \Mdwarf from \wobble (black) and from the \HARPS closed-source pipeline (red). \todo{(Say more about expected/observed secular evolution)}}
...
\caption{Comparison of telluric spectrum derived by \wobble analysis of 51 Peg spectra (blue) and the spectrum observed with a telluric standard star (black).}
...
Applicability of \wobble as shown here to current and near-future instruments.
