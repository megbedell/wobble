...
% Aims
Here we propose a data-driven method to simultaneously extract precise \RVs and infer the underlying stellar and telluric spectra using a linear model (in the log flux). 
...
\wobble implicitly assumes Gaussian noise in $\ln$ flux, which is not strictly accurate. 
...
Our method may also fail at low spectral resolution. \todo{(what will happen for undersampled lines?)}
...
\todo{(HOGG: say things about hacks)}
...
Another way in which the \wobble model may fail for some data sets is in its assumptions about spectral variability.
...
Perhaps the most obviously sub-optimal part of \wobble is its reliance on regularization. 
...
Another approach would be to take a non-parameteric approach like a Gaussian process (as \code{STARFISH} uses). 
A third approach would be to use a model that incorporates more physics: this could range from a model that is parameterized in terms of absorption line profile parameters to a complex model including various atomic and molecular physics of the lines. 
